import { RemoteData, pending, failure, success } from "./remote-data";
...
export class ApiClient {
  ...
  readonly request = <Response = never>(
    request: AjaxRequest
  ): Observable<RemoteData<AjaxError, Response>> => {
    ...
    return ajax(xhr).pipe(
      ...
      catchError(response => {
        ...
        console.log("Failed", failure(response));
  ...
  readonly remove = <Response = never>(
    url: string,
    query = {}
  ): Observable<RemoteData<AjaxError, Response>> => {
    return this.request({
